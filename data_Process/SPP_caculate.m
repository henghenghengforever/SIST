...
kyd = k0*sqrt(epsilon_d*epsilon_d/(epsilon_d+epsilon_m)); 
kym = k0*sqrt(epsilon_m*epsilon_m/(epsilon_d+epsilon_m));
L_yd = 1/abs(imag(kyd))*1e6; % the depth of SPP penetration in dieletric
L_ym = 1/abs(imag(kym))*1e6; % the depth of SPP penetration in matal
